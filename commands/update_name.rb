...
def update_name(str, json)
  ...
  logger.info "Successfully changed name to \"#{user.name}\" (triggered by #{json[:user][:screen_name]})"
rescue
  twitter.update("@#{json[:user][:screen_name]} 表示名を更新できませんでした(∩´﹏`∩)",
                 in_reply_to_status_id: json[:id])
  logger.info "Failed to change name to \"#{str}\" (triggered by #{json[:user][:screen_name]})"
end
